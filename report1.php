...
$dateval			= @$_REQUEST["timestamp"];//print_r($_POST);
$IMEI				= @$_REQUEST["imei"];
//echo"select min(id) as `minid`, max(id) as `maxid` from tbl_event_data where date(timestamp) = '".$dateval."' ";
...
//print_r($maxminid);
//  echo"select DeviceName,DeviceIMEI,DeviceType,driver_name from tbl_device_det where DeviceIMEI = '".$IMEI."' order by DeviceType";
			$result= mysql_query("select DeviceName,DeviceIMEI,DeviceType,driver_name from tbl_device_det where DeviceIMEI = '".$IMEI."' order by DeviceType ");//echo $result;die();
			while ($row = mysql_fetch_array($result)) { //echo "sdgksad";
				//print_r($row);die();
    ...
    //echo "select deviceid as 'Device Name',
